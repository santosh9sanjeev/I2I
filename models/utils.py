...
#     # print('hellllllooooo')
#     if hasattr(model, 'G_A') and hasattr(model, 'G_B'):
...
#         real_dir = os.path.join(opt.dataroot, 'valB')
#     else:
#         test_loader = test_loader_b
#         real_dir = os.path.join(opt.dataroot, 'valA')

#     for it, data in enumerate(test_loader):
#         fake = model.netG_A(data['A'])#.cuda())
#         # print('fakeeeeeeeeeeeeeeeeeee')
#         path_fake = os.path.join(fake_dir, os.path.basename(data['A_paths'][0]).replace('nii.gz', 'png'))
#         im = tensor2im(fake)
#         save_image_numpy(im, path_fake)
#     # print('itttttttttttttttttttttttttttttttt',it)
...
#     # print(realB_path)
#     # print(fakeB_path)
#     # realB_path = '/share/ssddata/CrossMoDA_cleaned_dataset_v2/valB_png'
#     realB_path = '/share/ssddata/CrossMoDA_cleaned_dataset_v2/valB_png_256'
...
#     else:
#         eval_dict = eval_maps(realB_path, fakeB_path)
...
#     # for filename in os.listdir(fakeB_path):
#     #     file_path = os.path.join(fakeB_path, filename)
...
#     realA_dir, realB_dir = os.path.join(opt.dataroot, 'valA'), os.path.join(opt.dataroot, 'valB')
...
#         eval_dict = eval_city2parsing(realB_path, fakeB_path)
#     eval_args = {'fid': True, 'kid': True, 'kid_subset_size': 50, 'kid_subsets': 10, 'verbose': False, 'cuda': False}
#     metric_dict_AB = torch_fidelity.calculate_metrics(input1=realB_path, input2=fakeB_path, **eval_args)
...
#         # real = nib.load(reals[i])
#         # real = real.get_fdata()
#         # eta = 1e-10
#         # real = (real - real.min())/(real.max()-real.min() + eta) * 255
...
#     # print('completeddddd hereeeeeeeeeeeee')
#     return eval_dict
...
def save_image_numpy(image_numpy, image_path, aspect_ratio=1.0):
    ...
    if aspect_ratio is None:
        ...
        # image_pil = image_pil.resize((h, int(w * aspect_ratio)), Image.BICUBIC)
        image_pil = image_pil.resize((350,350), Image.BICUBIC)
    elif aspect_ratio < 1.0:
        # image_pil = image_pil.resize((int(h / aspect_ratio), w), Image.BICUBIC)
        image_pil = image_pil.resize((350,350), Image.BICUBIC)
...
def eval_method_one(realB_path, fakeB_path, opt):
    print('[*] start evaluation!')
    realB_path = '/share/ssddata/CrossMoDA_final_dataset_before_crop_for_GANs/valB_png'
...
def eval_maps(real_path, fake_path, thr1=5, thr2=10, name=''):
    ...
    fakes = sorted(fakes)
    # print(real_path, fake_path)
    # print('realllllllllllllllllllllls',reals)
    num_imgs = len(fakes)
    # print(num_imgs)
    corr5_count = 0.0
    corr10_count = 0.0
    ...
    for i in range(num_imgs):
        ...
        real = cv2.imread(reals[i],0)
        fake = cv2.imread(fakes[i],0)
        # print(real.shape, fake.shape)
        real = cv2.resize(real, (350, 350), interpolation=cv2.INTER_LINEAR)
        fake = cv2.resize(fake, (350, 350), interpolation=cv2.INTER_LINEAR)
        # print(real.shape, fake.shape)
